...
from Bigger_Agent import Agent
import collections
import torch
#functions section
...
path = "agent_checkpoint_1.pth"

checkpoint = torch.load(path, map_location=torch.device('cpu'), weights_only=False)

model.local_network.load_state_dict(checkpoint['local_network_state_dict'])
model.promotion_network.load_state_dict(checkpoint['promotion_network_state_dict'])
model.optimiser.load_state_dict(checkpoint['optimiser_state_dict'])
model.promotion_network_optimiser.load_state_dict(checkpoint['promotion_optimiser_state_dict'])



model.local_network.eval()
model.promotion_network.eval()

print("Model, optimizers, memory, and t_step loaded successfully!")
...
try :
    for i in range(total_moves_to_make):
        ...
        if currSide==0:
            ...
            fen = obs.fen()
            opponent_move = random.choice(env.legal_moves)
